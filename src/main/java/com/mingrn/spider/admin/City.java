...
public class City implements Administrative {
    ...
    final AdministrativeLevel CITY = AdministrativeLevel.CITY;
    ...
    public List<Node> parsingHtml(Document html, String url) {
        ...
        Elements tables = html.getElementsByClass(CITY.level + TABLE);
        if (tables.isEmpty()) {
            return null;
        }
        ...
        Elements trs = table.getElementsByClass(CITY.level + TR);
        for (Element tr : trs) {
            ...
            if (href != null && !"".equals(href)){
                ...
                nextUrl += href;
                List<Node> children = new SpiderExecutor().execute(CITY.nextLevel(), nextUrl);
